...
class ContentAnalyzer:
    ...
    def analyze_content(file_path: Path) -> Dict[str, Any]:
        """Analizar contenido con detección mejorada para anime vintage"""
        results = {
            "type": "unknown",
            "characteristics": [],
    ...
    def _analyze_image_adjusted(img: Image.Image) -> Dict[str, Any]:
        """Análisis con lógica ajustada específicamente para casos como One Piece"""
        results = {
            "type": "unknown",
            "characteristics": [],
            "recommended_model": "realesrgan-x4plus",
    ...
    def _detect_anime_characters(img_array: np.ndarray) -> float:
        """Detectar personajes anime con algoritmos mejorados"""
        try:
    ...
    def _detect_anime_color_style(img_array: np.ndarray) -> float:
        """Detectar estilo de colores típico del anime"""
        try:
            hsv = cv2.cvtColor(img_array, cv2.COLOR_RGB2HSV)
            h, s, v = cv2.split(hsv)
    ...
    def _detect_anime_line_art(img_array: np.ndarray) -> float:
        """Detectar líneas características del anime"""
        try:
            gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
    ...
    def _detect_vintage_quality(img_array: np.ndarray) -> float:
        """Detectar características de calidad vintage (simplificado)"""
        try:
            h, w = img_array.shape[:2]
            vintage_score = 0.0
    ...
    def _detect_modern_quality(img_array: np.ndarray) -> float:
        """Detectar características de calidad moderna (simplificado)"""
        try:
            h, w = img_array.shape[:2]
            modern_score = 0.0
    ...
    def _detect_gaming_ui(img_array: np.ndarray) -> float:
        """Detectar UI gaming (simplificado)"""
        try:
            gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
            edges = cv2.Canny(gray, 50, 150)
            contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def _analyze_contrast_gaming(img_array: np.ndarray) -> float:
        """Analizar contraste gaming (simplificado)"""
        try:
            gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
    ...
    def _analyze_natural_noise(img_array: np.ndarray) -> float:
        """Detectar ruido natural (simplificado)"""
        try:
            gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
    ...
    def _analyze_organic_textures(img_array: np.ndarray) -> float:
        """Detectar texturas orgánicas (simplificado)"""
        try:
            gray = cv2.cvtColor(img_array, cv2.COLOR_RGB2GRAY)
    ...
    def _merge_frame_results_adjusted(all_scores: List[Dict]) -> Dict[str, Any]:
        """Fusión ajustada que prioriza anime"""
        if not all_scores:
            return {}
