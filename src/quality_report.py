"""
quality_report.py - Before/after quality report generator.

Computes image quality metrics (SSIM, sharpness, noise, color gamut, file size)
...
class QualityReportSystem:
    ...
    def _analyze_image_quality(self, file_path: Path) -> dict:
        ...
        try:
            ...
            # Guardar array de frame para SSIM inter-imagen
            metrics["_frame_array"] = img_array
    ...
    def _compute_ssim(self, img1: np.ndarray, img2: np.ndarray) -> float:
        """SSIM simplificado usando numpy — sin dependencia extra."""
    ...
    def _calculate_improvements(self, original: dict, processed: dict) -> dict:
        ...
        try:
            # SSIM entre primer frame original y procesado
            try:
                orig_arr = original.get("_frame_array")
                proc_arr = processed.get("_frame_array")
                if orig_arr is not None and proc_arr is not None:
                    orig_gray = cv2.cvtColor(orig_arr, cv2.COLOR_RGB2GRAY)
                    proc_gray = cv2.cvtColor(proc_arr, cv2.COLOR_RGB2GRAY)
                    improvements["ssim"] = self._compute_ssim(orig_gray, proc_gray)
            except Exception:
                pass
    ...
    def _create_file_info_comparison(self, parent, report: dict):
        ...
        data_rows = [
            ...
            ("SSIM",
             "—",
             "—",
             f"{improvements.get('ssim', 0):.4f}" if "ssim" in improvements else "N/A")
        ]
